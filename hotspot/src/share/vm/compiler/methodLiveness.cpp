...
//
// 1. We do not do the analysis necessary to match jsr's (jsr : jump to subroutine) with the appropriate ret.
...
// Identifying _gen, _kill locals within a given range
void MethodLiveness::BasicBlock::compute_gen_kill_range(ciBytecodeStream *bytes) {
  _gen.clear();
  _kill.clear();
// For each bytecode, identifying _gen, _kill locals
